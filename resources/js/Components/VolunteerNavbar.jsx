...
export default function VolunteerNavbar() {
  ...
  const items = [
    { label: 'Dashboard', route: 'dashboard.relawan' },
    { label: 'Lowongan', route: 'jobs.index' },
    { label: 'Peluang', route: 'volunteer.opportunities' },
    { label: 'Forum', route: 'posts.index' },
    { label: 'Acara', route: 'events.index' },
    { label: 'Profil', route: 'volunteer.profile' },
    { label: 'Sertifikat', route: 'volunteer.certificates' },
    { label: 'Notifikasi', route: 'notifications.index' },
  ];
  const exists = (r) => route().has(r);
  const unread = notifications?.unread_count || 0;
  const accentKey = ROLE_ACCENTS['volunteer'] || 'emerald';
  const accentClasses = getAccentClasses(accentKey);

  return (
    <nav className={`fixed inset-x-0 top-0 z-50 border-b border-gray-200 ${accentClasses.bg.replace('bg-','bg-')} bg-white/95 backdrop-blur-sm shadow-sm`} aria-label="Volunteer primary">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="hidden md:flex items-center gap-1">
              {items.filter(i => exists(i.route)).map(i => {
                ...
                return (
                  <Link key={i.route} href={route(i.route)} className={`relative px-3 py-2 rounded-md text-sm font-medium transition-colors ${active ? 'bg-emerald-50 text-emerald-700' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'}`} aria-current={active ? 'page' : undefined}>
                    {i.label}
                    {isNotif && unread > 0 && (
                      <span className="absolute -top-1 -right-1 inline-flex items-center justify-center rounded-full bg-emerald-600 text-white text-[10px] px-1 min-w-[1.15rem] h-5">{unread}</span>
                    )}
                  </Link>
                );
              })}
        ...
        <div className="md:hidden border-t border-gray-100">
          <div className="px-2 py-3 space-y-1">
            {items.filter(i => exists(i.route)).map(i => {
              ...
              const isNotif = i.route === 'notifications.index';
              return (
                <Link key={i.route} href={route(i.route)} onClick={() => setOpen(false)} className={`flex items-center gap-2 px-3 py-2 rounded-md text-sm font-medium ${active ? 'bg-emerald-50 text-emerald-700' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'}`}>
                  {i.label}
                  {isNotif && unread > 0 && (
                    <span className="ml-auto inline-flex items-center justify-center rounded-full bg-emerald-600 text-white text-[10px] px-1 min-w-[1.15rem] h-5">{unread}</span>
                  )}
                </Link>
              );
            })}
